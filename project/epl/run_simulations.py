...
def run_netqasm_simulate(n_max, n_succ):
    ...
    for arg in range(len(fidelities_to_run)):
        ...
        update_all_link_fidelities("network.yaml", fid)


        #Create a unique file name to store the data
        title_fid = titles[arg]
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"EPL_depol_{title_fid}_{timestamp}.txt"
        ...
        while i < n_max:
            ...
            #Determine number of succesfull entanglement generations, if 25, break
...
if __name__ == "__main__":
    n_max = 250 # Change to how many times you want to run the simulation
    n_succ = 25
